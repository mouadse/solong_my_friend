...
void	render_map_tile(t_game_state *state, int y, int x)
{
	...
	if (state->map.layout[x][y] == '1')
		mlx_put_image_to_window(state->mlx_instance, state->window_instance,
			state->assets.wall_asset, x * asset_size, y * asset_size);
	else if (state->map.layout[x][y] == '0')
		mlx_put_image_to_window(state->mlx_instance, state->window_instance,
			state->assets.floor_asset, x * asset_size, y * asset_size);
	else if (state->map.layout[x][y] == 'C')
		mlx_put_image_to_window(state->mlx_instance, state->window_instance,
			state->assets.coin_asset, x * asset_size, y * asset_size);
	else if (state->map.layout[x][y] == 'E')
	{
		if (state->game_completed)
			mlx_put_image_to_window(state->mlx_instance, state->window_instance,
				state->assets.open_door_asset, x * asset_size, y * asset_size);
		else
			mlx_put_image_to_window(state->mlx_instance, state->window_instance,
				state->assets.closed_door_asset, x * asset_size, y
				* asset_size);
	}
}

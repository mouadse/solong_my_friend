...
LEAKS=$?
if [ $LEAKS -eq 1 ]; then
        ft_printf "${RED}$C.[MKO] LEAKS ${DEF_COLOR}"
else
        ft_printf "${GREEN}$C.[MOK]${DEF_COLOR}";
fi
...
if [ $SO_LONG_EXIT_CODE == 139 ];then
        ft_printf "${RED}[KO] SEGFAULT${DEF_COLOR}\n";
        ((C++))
...
if [ $LINE == "Error" ];then
        ft_printf "${GREEN}[OK] ${DEF_COLOR}";
        if [ -n "$LINE2" ];then
                if [ "${#LINE2}" -gt 1 ]; then
                        ft_printf "${GRAY}\nExpected output: ${DEF_COLOR} $EXPECTED_OUTPUT \n"
                        ft_printf "${GRAY}Your output: ${DEF_COLOR} $LINE2 \n"
                else
                        ft_printf "${RED}[KO] You must display an explicit error message${DEF_COLOR}\n";
                        ft_printf "${GRAY}Expected output: ${DEF_COLOR} $EXPECTED_OUTPUT \n"
                fi
        else
                ft_printf "${RED}[KO] You must display an explicit error message${DEF_COLOR}\n";
                ft_printf "${GRAY}Expected output: ${DEF_COLOR} $EXPECTED_OUTPUT \n"
        ...
        ft_printf "${RED}[KO] Expected output: Error${DEF_COLOR}\n";
...
if [ $CHECK_ERROR == 0 ]; then
    ...
    ft_printf "${GREEN}$C.[OK] ${DEF_COLOR}\n";

else
    ft_printf "${RED}$C.[KO] ${DEF_COLOR}\n";
fi
...
if [ -f "$FILE" ]; then
	...
	ft_printf "${RED}NO EXIST SO_LONG PROGRAM ${DEF_COLOR}\n";
	exit 0
fi
...
if [ $LINE == "Error" ];then
        ...
        if [ -n "$LINE2" ];then
                if [ "${#LINE2}" -gt 1 ]; then
                        ...
                        ft_printf "${RED}[KO] You must display an explicit error message${DEF_COLOR}\n";
                        ft_printf "${GRAY}Expected output: ${DEF_COLOR} Wrong number of arguments \n"
                fi
        else
                ft_printf "${RED}[KO] You must display an explicit error message${DEF_COLOR}\n";
                ft_printf "${GRAY}Expected output: ${DEF_COLOR} Wrong number of arguments \n"
